...
const calculateDerivedValues = (obs) => {
  ...
  // Calculate global KPIs for CPG × Retail collaboration
  const joint_forecast_accuracy = globalKPIs['joint-forecast-accuracy'].formula(obs);
  const retailer_dc_service = globalKPIs['retailer-dc-service'].formula(obs);
  const on_shelf_availability = globalKPIs['on-shelf-availability'].formula(obs);
  const inventory_risk_corridor = globalKPIs['inventory-risk-corridor'].formula(obs);
  const total_cost_to_serve = globalKPIs['total-cost-to-serve'].formula(obs);
  ...
  return {
    ...
    // Global cross-organization KPIs
    joint_forecast_accuracy,
    retailer_dc_service,
    on_shelf_availability,
    inventory_risk_corridor,
    total_cost_to_serve,
  };
};
...
// ----------------------------------------------------------------------------
// CPG × RETAIL COLLABORATIVE STAKEHOLDER ECOSYSTEM
// Dual-organization system based on Sandra Mitchell's complexity theory
// ----------------------------------------------------------------------------
  ...
  cpg: {
    ...
    roles: {
      'demand-planner': {
        ...
        metadata: ['consensus_forecast_qty', 'forecast_error_dist']
      },
      'inventory-planner': {
        ...
        metadata: ['fg_opening_stock', 'target_cof_pct', 'holding_cost_rate']
      },
      'production-scheduler': {
        ...
        metadata: ['planned_production_qty', 'mfg_adherence_pct', 'production_lot_size']
      },
      'supply-planner': {
        ...
        metadata: ['open_po_qty_rpm', 'supplier_otif_dist', 'lead_time_variability']
      },
      'sales-kam': {
        name: 'Sales/KAM',
        avatar: '🤝',
        ...
        metadata: ['order_variability_cv', 'cost_of_failure_unit']
      }
    }
  },
  retail: {
    ...
    roles: {
      'category-manager': {
        ...
        retailMetadata: ['category_growth_rate', 'promo_roi', 'osa_target']
      },
      'replenishment-planner': {
        ...
        retailMetadata: ['dc_service_level', 'replenishment_frequency', 'store_osa']
      },
      'retail-demand-planner': {
        ...
        retailMetadata: ['pos_forecast_accuracy', 'baseline_uplift_split', 'substitution_rate']
      },
      'buyer-procurement': {
        ...
        retailMetadata: ['moq_compliance', 'delivery_window_adherence', 'fill_rate_cpg']
      },
      'dc-operations': {
        name: 'DC Operations',
        avatar: '🏗️',
        ...
        retailMetadata: ['inbound_capacity_util', 'slotting_efficiency', 'pick_rate']
      }
    }
  }
};
...
const globalKPIs = {
  'joint-forecast-accuracy': {
    ...
    impactedBy: ['demand-planner', 'retail-demand-planner', 'category-manager'],
    formula: (obs) => {
      // Simple calculation for demo - would be more complex in production
      const cpgAccuracy = 1 - obs.forecast_error_dist.value;
      const retailAssumedAccuracy = 0.82; // From retail POS forecast
      return (cpgAccuracy + retailAssumedAccuracy) / 2;
    }
  },
  'retailer-dc-service': {
    ...
    impactedBy: ['inventory-planner', 'replenishment-planner', 'supply-planner'],
    formula: (obs) => {
      // Derived from supplier OTIF and manufacturing adherence
      return obs.supplier_otif_dist.value * obs.mfg_adherence_pct.value * 0.98;
    }
  },
  'on-shelf-availability': {
    ...
    impactedBy: ['category-manager', 'dc-operations', 'replenishment-planner'],
    formula: (obs) => {
      // Simplified - accounts for DC service and retail execution
      const dcService = obs.supplier_otif_dist.value * 0.98;
      const retailExecution = 0.96; // Assumed retail DC-to-store performance
      return dcService * retailExecution;
    }
  },
  'inventory-risk-corridor': {
    ...
    impactedBy: ['inventory-planner', 'replenishment-planner', 'production-scheduler'],
    formula: (obs) => {
      const fg = obs.fg_opening_stock.value;
      const retailAssumed = fg * 0.15; // Retailer holds ~15% of CPG inventory
      return {
        p10: (fg + retailAssumed) * 0.9,
        p50: fg + retailAssumed,
        p90: (fg + retailAssumed) * 1.14
      };
    }
  },
  'total-cost-to-serve': {
    ...
    impactedBy: ['supply-planner', 'buyer-procurement', 'dc-operations'],
    formula: (obs) => {
      // Simplified cost calculation
      const holdingCost = obs.holding_cost_rate.value * 100;
      const serviceFailureCost = (1 - obs.supplier_otif_dist.value) * 20;
      return holdingCost + serviceFailureCost;
    }
  }
};
...
const complexityScenarios = {
  'promo-cascade': {
    ...
    causationChain: [
      ...
      { node: 'pos-spike', type: 'market-signal', causedBy: 'promo-plan', impact: 2.3, label: 'POS Demand Spike +230%' },
      { node: 'dc-strain', type: 'operational-stress', causedBy: 'pos-spike', impact: 1.8, label: 'DC Capacity Strain' },
      { node: 'cpg-emergency-production', type: 'cpg-response', causedBy: 'dc-strain', impact: 1.5, label: 'Emergency Production Run' },
      { node: 'supplier-pressure', type: 'upstream-stress', causedBy: 'cpg-emergency-production', impact: 1.2, label: 'Supplier Lead Time Pressure' }
    ],
    globalKPIImpact: {
  ...
  'supply-disruption': {
    ...
    causationChain: [
      ...
      { node: 'rpm-shortage', type: 'material-risk', causedBy: 'supplier-delay', impact: 0.78, label: 'RPM Shortage Risk' },
      { node: 'production-constraint', type: 'manufacturing-impact', causedBy: 'rpm-shortage', impact: 0.85, label: 'Production Constrained' },
      { node: 'fg-stockout', type: 'service-risk', causedBy: 'production-constraint', impact: 0.45, label: 'FG Stockout Risk' },
      { node: 'retailer-osa-decline', type: 'retail-impact', causedBy: 'fg-stockout', impact: 0.88, label: 'Store OSA Decline' }
    ],
    globalKPIImpact: {
  ...
  'forecast-misalignment': {
    ...
    causationChain: [
      ...
      { node: 'forecast-gap', type: 'planning-conflict', causedBy: ['cpg-forecast', 'retail-forecast'], impact: 1.25, label: 'Forecast Gap Detected' },
      { node: 'inventory-mismatch', type: 'operational-inefficiency', causedBy: 'forecast-gap', impact: 1.15, label: 'Inventory Position Mismatch' },
      { node: 'service-degradation', type: 'performance-impact', causedBy: 'inventory-mismatch', impact: 0.92, label: 'Service Level Degradation' }
    ],
    globalKPIImpact: {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="flex items-center gap-1 bg-slate-800 rounded-lg p-1">
              {[
                ...
                { id: 'collaborative', label: '🤝 Collaborative Room', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeView === 'collaborative' && (
          <CollaborativeDecisionRoom currentState={currentState} observations={observations} />
        )}
...
// ============================================================================
// CPG × RETAIL COLLABORATIVE COMPONENTS
// ============================================================================

// Stakeholder Card Component
const StakeholderCard = ({ stakeholder, organization, active, mandatory, onToggle }) => {
  const colors = {
    cpg: { bg: 'bg-cyan-950', border: 'border-cyan-600', text: 'text-cyan-300', dot: 'bg-cyan-500' },
    retail: { bg: 'bg-emerald-950', border: 'border-emerald-600', text: 'text-emerald-300', dot: 'bg-emerald-500' }
  };
  const c = colors[organization];

  return (
    <button
      onClick={onToggle}
      disabled={mandatory}
      className={`w-full p-3 rounded-lg border-2 ${active ? c.bg : 'bg-slate-800'} ${active ? c.border : 'border-slate-700'} text-left transition-all ${
        mandatory ? 'opacity-100 cursor-not-allowed' : 'hover:scale-[1.02] cursor-pointer'
      }`}
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className={`text-sm font-medium ${c.text}`}>{stakeholder.name}</div>
            {mandatory && <div className="text-xs text-amber-400">✓ Mandatory</div>}
          </div>
        </div>
        {active && <div className={`w-3 h-3 rounded-full ${c.dot}`} />}
      </div>
      <div className="text-xs text-slate-400">
        {stakeholder.expertise.slice(0, 2).join(' • ')}
      </div>
    </button>
  );
};
...
const GlobalKPICard = ({ kpiKey, kpi, currentValue, impactLevel }) => {
  const formatValue = (val) => {
    if (kpiKey.includes('accuracy') || kpiKey.includes('service') || kpiKey.includes('availability')) {
      return `${(val * 100).toFixed(1)}%`;
    }
    if (kpiKey.includes('cost')) {
      return `${val.toFixed(1)}%`;
    }
    if (kpiKey === 'inventory-risk-corridor' && typeof val === 'object') {
      return `P50: ${(val.p50 / 1000).toFixed(1)}K`;
    }
    return val.toFixed(2);
  };

  const getStatusColor = () => {
    if (!kpi.target) return 'text-slate-400';
    const diff = Math.abs(currentValue - kpi.target);
    const tolerance = kpi.target * 0.1;
    if (diff < tolerance) return 'text-emerald-400';
    if (diff < tolerance * 2) return 'text-amber-400';
    return 'text-red-400';
  };
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {impactLevel && (
          <div className={`text-xs px-2 py-0.5 rounded ${
            impactLevel === 'high' ? 'bg-red-900 text-red-400' :
            impactLevel === 'medium' ? 'bg-amber-900 text-amber-400' :
...
// Collaborative Decision Room Component
const CollaborativeDecisionRoom = ({ currentState, observations }) => {
  const [activeStakeholders, setActiveStakeholders] = useState({
    cpg: ['demand-planner'],
    retail: ['category-manager', 'replenishment-planner']
  });
  const [selectedScenario, setSelectedScenario] = useState(null);

  const handleStakeholderToggle = useCallback((org, key) => {
    const role = stakeholderEcosystem[org].roles[key];
    if (role?.mandatory) return;

    setActiveStakeholders(prev => ({
      ...prev,
      [org]: prev[org].includes(key)
        ? prev[org].filter(k => k !== key)
        : [...prev[org], key]
    }));
  }, []);

  const calculateImpactLevel = (kpi) => {
    const impactingCount = kpi.impactedBy.filter(role =>
      activeStakeholders.cpg.includes(role) || activeStakeholders.retail.includes(role)
    ).length;
    const ratio = impactingCount / kpi.impactedBy.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Global KPIs (System-Wide Performance)</h3>
        <div className="grid grid-cols-3 gap-4">
          {Object.entries(globalKPIs).map(([key, kpi]) => (
            <GlobalKPICard
              key={key}
              kpiKey={key}
              kpi={kpi}
              currentValue={currentState[key.replace(/-/g, '_')] || kpi.current}
              impactLevel={calculateImpactLevel(kpi)}
            />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
          ...
          <div className="space-y-3">
            {Object.entries(stakeholderEcosystem.cpg.roles).map(([key, stakeholder]) => (
              <StakeholderCard
                key={key}
                stakeholder={stakeholder}
                organization="cpg"
                active={activeStakeholders.cpg.includes(key)}
                onToggle={() => handleStakeholderToggle('cpg', key)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
          ...
          <div className="space-y-3">
            {Object.entries(stakeholderEcosystem.retail.roles).map(([key, stakeholder]) => (
              <StakeholderCard
                key={key}
                stakeholder={stakeholder}
                organization="retail"
                active={activeStakeholders.retail.includes(key)}
                mandatory={stakeholder.mandatory}
                onToggle={() => handleStakeholderToggle('retail', key)}
              />
            ))}
          </div>
        </div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Complexity Scenarios</h3>
        <div className="grid grid-cols-3 gap-4 mb-6">
          {Object.entries(complexityScenarios).map(([key, scenario]) => {
            const severity = Object.values(scenario.globalKPIImpact).some(v => Math.abs(v) > 2) ? 'high' : 'medium';
            const colors = {
              high: 'bg-red-950 border-red-600 text-red-400',
              medium: 'bg-amber-950 border-amber-600 text-amber-400'
            };

            return (
              <div key={key} className={`p-4 rounded-lg border-2 ${colors[severity]}`}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">⚠️</span>
                  <div className="font-semibold text-sm">{scenario.name}</div>
                </div>
                <div className="text-xs opacity-90 mb-2">{scenario.description}</div>
                <div className="space-y-1">
                  {Object.entries(scenario.globalKPIImpact).slice(0, 2).map(([kpi, impact]) => (
                    <div key={kpi} className="text-xs flex items-center gap-1">
                      <span className="opacity-75">{globalKPIs[kpi]?.name.split(' ')[0]}:</span>
                      <span className="font-mono font-bold">{typeof impact === 'number' ? (impact > 0 ? '+' : '') + impact.toFixed(1) : impact}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {selectedScenario && (
          <div className="bg-slate-800 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-white mb-3">
              Causation Chain: {complexityScenarios[selectedScenario].name}
            </h4>
            <div className="space-y-2">
              {complexityScenarios[selectedScenario].causationChain.map((node, idx) => (
                <div key={idx}>
                  <div className="p-3 bg-slate-900 rounded-lg border border-slate-700">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {node.impact && (
                        <div className={`text-xs px-2 py-0.5 rounded ${
                          node.impact > 2 ? 'bg-red-900 text-red-400' :
                          node.impact > 1 ? 'bg-amber-900 text-amber-400' :
                          'bg-emerald-900 text-emerald-400'
                        }`}>
                          {node.impact.toFixed(1)}x
                        </div>
                      )}
                    </div>
                    <div className="text-xs text-slate-500">
                      {node.type} • {node.owner || 'System'}
                    </div>
                  </div>
                  {idx < complexityScenarios[selectedScenario].causationChain.length - 1 && (
