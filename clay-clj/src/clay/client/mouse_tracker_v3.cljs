(ns clay.client.mouse-tracker-v3
  "Mouse Tracker v3 - Using Sente for WebSocket communication.
   Features:
   ...
   - Sente for bidirectional WebSocket communication"
...
(defn init-sente!
  "Initialize Sente channel socket client"
  []
  (println "Initializing Sente client...")
  (let [{:keys [chsk ch-recv send-fn state]}
    ...
    (reset! !sente {:chsk chsk
                    ...
                    :send-fn send-fn
                    :state state})
    (println "Sente client initialized")))

(defn send-coords!
  "Send mouse coordinates to server via Sente"
  [coords]
  (when-let [{:keys [send-fn]} @!sente]
    (send-fn [:mouse/coords coords])))
...
(defn ui-view
  ...
  [{:keys [coords status status-class]}]
  [:div.container
   [:div.title "Mouse Tracker v3"]
   [:div.subtitle "Sente + Missionary + Replicant"]
   [:div.coords-display
...
(defn start-sente-router!
  ...
  (when-let [{:keys [ch-recv]} @!sente]
    (println "Starting Sente client router...")
...
(defn init!
  ...
  []
  (println "Initializing Mouse Tracker v3...")
  (inject-styles!)
  (render-ui!)
  (init-sente!)
  (start-sente-router!)
  (start-mouse-tracking!)
  (println "Mouse Tracker v3 initialized"))

(defn reload!
  "Hot reload handler for shadow-cljs"
  []
  (println "Reloading Mouse Tracker v3...")
  (stop-mouse-tracking!)
  (render-ui!)
